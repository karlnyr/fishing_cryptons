...
echo "3 - Initate final join on taxa id to taxa name"
join -t $'\t' \
    -1 3 -2 1 \
    -o 1.1,1.2,1.3,2.4 \
    <(sort -t $'\t' -k3,3 $OUT_FILE_PATH/$OUT_NAME$FILT_EXT) \
    <(sort -t $'\t' -k1,1 $ID_TO_NAME_PATH) \
    > $OUT_FILE_PATH/$OUT_NAME$FILT_BLAST
echo "3 - Done"

echo "4 - Filtering out unique protein and taxa combinations"
cut -f1,2,4 $OUT_FILE_PATH/$OUT_NAME$FILT_BLAST \
    | uniq \
    | sort -k1 > $OUT_FILE_PATH/$OUT_NAME$UINIQ_COMB
echo "4 - Done"

echo "5 - Extracting format 6 information of filtered proteins"
python3 $PY_07_PATH \
    $REPBASE \
